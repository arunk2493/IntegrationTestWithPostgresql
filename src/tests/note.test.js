...
describe('Note route', function () {
    ...
    before('Mock db connection and load app', async function () {
        ...
        await client.query('CREATE TABLE note (id SERIAL NOT NULL ,name TEXT UNIQUE NOT NULL ,content TEXT NOT NULL )')
    })
    ...
    after('Mock Table drop',async function (){
        await client.query('DROP TABLE note');
    })*/
    ...
    describe('POST /note', function () {
       ...
       /* it('Should fail if name already exists', async function () {
            ...
            await postNote(req)
            await postNote(req, 200) // Second request should fail
        })

        it('Should fail if request is missing required params', async function () {
            await postNote({ name: 'note12' }, 400)
            await postNote({ content: 'content12' }, 400)
            await postNote({}, 400)
        })*/
    })
